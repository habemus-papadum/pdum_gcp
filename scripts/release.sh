...
"""Release automation script for gcp.
...
INIT_PY = REPO_ROOT / "src" / "pdum" / "gcp" / "__init__.py"
...
def main() -> None:
    ...
    # Parse arguments
    parser = argparse.ArgumentParser(description="Automate the release process for gcp")
    parser.add_argument(
        "bump_level",
    ...
    # Show title
    title = Panel.fit(
        "[bold cyan]gcp Release Script[/bold cyan]\n\n"
